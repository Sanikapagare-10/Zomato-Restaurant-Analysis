...
# Since your notebook is inside the 'notebook' folder, this script runs from the project root.
# Adjust the path to 'dataset/zomato.csv' if you run it from the root.
try:
    df = pd.read_csv("dataset/zomato.csv")
except FileNotFoundError:
    df = pd.read_csv("../dataset/zomato.csv")

# Clean column names to match SQL schema variables exactly
...
conn = mysql.connector.connect(
    host="localhost",
    user="root",
    password="S@nika105",  # ⬅️ REPLACE THIS WITH YOUR ACTUAL MYSQL PASSWORD
    database="zomato_db"
)
cursor = conn.cursor()
...
# Construct SQL Query string mapping out all 17 fields
...
# Format all rows cleanly (replacing pandas NaN null flags with clean SQL None elements)
records = [[None if pd.isna(val) else str(val) for val in row] for row in df.values]

# --- BATCH UPLOAD LOGIC TO PREVENT PACKET DISCONNECTS ---
batch_size = 2000
...
for i in range(0, total_records, batch_size):
    ...
    conn.commit()  # Save progress per batch
